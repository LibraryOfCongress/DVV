...
<xsl:template match="alto:processingStepSettings">
    ...
    <br/>
    <xsl:if test="contains(text(), 'Predicted')">
    	<xsl:variable name="after_predicted" select="substring-after(. , 'Predicted')"/>
    	<xsl:variable name="word" select="substring-after($after_predicted, 'Word')"/>
    	<xsl:variable name="accuracy" select="substring-after($word, 'Accuracy')"/>
    	<xsl:variable name="value" select="substring-after($accuracy, ':')"/>
    	<xsl:variable name="pwa" select="substring-before($value, ' ')"/>
    	<br/>
	<b>Predicted Word Accuracy:<xsl:value-of select="$pwa"/></b>
    </xsl:if>
...
<xsl:template match="alto:ComposedBlock">
        <h3>Composed Block - ID: <xsl:value-of select="@ID"/></h3>
        <!-- <p>Accuracy Threshold: <xsl:value-of select="$ocrPercent"/></p> -->
        <table border="1">
            <tr>
                <th>StyleRefs</th>
